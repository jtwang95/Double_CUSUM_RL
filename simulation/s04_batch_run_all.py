...
python s02_test_kappa.py -B $B -p $P --kappa $kappa --ts $ts -M $M --htype $htype \
    ...
    --pt-hidden-dims $pt_hidden_dims --pt-epochs $pt_epochs --pt-cv $pt_cv --pt-cv-folds $pt_cv_folds \
    --pt-cv-cores $pt_cv_cores --pt-nn-candidates $pt_nn_candidates --pt-epochs-candidates $pt_epochs_candidates \
    ...
    with open(sbatch_filename, "w") as f:
        f.write(
            tpl2.format(B, M, P, HTYPE, NUM_RANDOM_REPEATS, OUT_FOLDER, kappa,
                        ...
                        PT_HIDDEN_DIMS, PT_EPOCHS, PT_CV, PT_CV_FOLDS,
                        PT_CV_CORES, PT_NN_CANDIDATES, PT_EPOCHS_CANDIDATES,
                        NUM_WCOMPONENTS, GAMMA, JOB_ID))
    p = subprocess.Popen("bash " + sbatch_filename,
                         shell=True,
                         stdout=subprocess.PIPE)
    p.wait()
